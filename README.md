...
- **Butterworth 低通滤波**：二阶 IIR 滤波器（35Hz 截止频率），滤除陀螺仪高频噪声
- **Slew Rate 斜坡限制**：限制角速度目标的变化率，避免阶跃冲击
- **故障保护**：角度/角速度超限自动停转（failsafe）
- **IBUS 遥控器协议**：支持 FlySky FS-i6 14通道数据解析
- **蓝牙实时数据**：通过串口 DMA 发送调试数据（12通道 CSV 格式），可用 SerialPlot 或上位机实时查看
- **NRF24L01 无线通信**：预留接口（代码已集成，待测试）

## 控制架构

```
遥控器(IBUS) ──→ 角度外环PID(100Hz) ──→ 角速度内环PID(200Hz) ──→ 电机混控 ──→ PWM输出
                      ↑                        ↑
                 DMP姿态角              陀螺仪+低通滤波
                      ↑                        ↑
                 MPU6050(DMP) ──────── MPU6050(原始数据)
```
...
│   ├── mpu6050/        # MPU6050 驱动（I2C读写 + DMP接口）
│   │   ├── biquad_filter/  # 二阶 Butterworth 低通滤波器
│   │   └── eMPL/       # InvenSense 官方 DMP 驱动库
│   ├── motor/          # 电机PWM输出（TIM5 四通道）
│   ├── ibus/           # FlySky IBUS 协议解析
│   ├── nrf24l01/       # NRF24L01 2.4G无线模块驱动
│   ├── pwm/            # PWM初始化
│   ├── iic/            # 软件I2C驱动
│   ├── led/            # LED指示
│   ├── key/            # 按键输入
...
- 2025年3月：搭建硬件平台，完成 MPU6050 驱动和 DMP 初始化
- 2025年4月：实现 IBUS 遥控器协议解析，PWM 电机驱动
- 2025年4-5月：串级 PID 控制器开发与调参，增加低通滤波和保护逻辑
- 2025年5-6月：蓝牙串口调试功能，大量调参测试（见姿态参数数据）
- 2025年6月：代码整理，准备上传 GitHub
