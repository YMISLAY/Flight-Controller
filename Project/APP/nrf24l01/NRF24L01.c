...
{
    HAL_GPIO_WritePin(NRF24L01_CE_PORT, NRF24L01_CE_PIN, 
...
{
    HAL_GPIO_WritePin(NRF24L01_CSN_PORT, NRF24L01_CSN_PIN, 
                     BitValue ? GPIO_PIN_SET : GPIO_PIN_RESET);
}
...
{
    HAL_GPIO_WritePin(NRF24L01_SCK_PORT, NRF24L01_SCK_PIN, 
                     BitValue ? GPIO_PIN_SET : GPIO_PIN_RESET);
}
...
{
    HAL_GPIO_WritePin(NRF24L01_MOSI_PORT, NRF24L01_MOSI_PIN, 
                     BitValue ? GPIO_PIN_SET : GPIO_PIN_RESET);
}
...
{
    return (HAL_GPIO_ReadPin(NRF24L01_MISO_PORT, NRF24L01_MISO_PIN) == GPIO_PIN_SET) ? 1 : 0;
}
...
void NRF24L01_GPIO_Init(void)
{
    ...
    /* 对于STM32F4，如果需要禁用JTAG以释放PA15，使用以下代码 */
    /* 注意：这会禁用JTAG，只保留SWD调试接口 */
    /*
    ...
    */
    ...
    /* 或者使用更精确的引脚重映射（推荐） */
    /* 禁用JTAG，释放PA15 (JTDI), PB3 (JTDO/TRACESWO), PB4 (NJTRST) */
    /* 但保留SWD调试接口（PA13, PA14） */
    /*
    __HAL_RCC_AFIO_CLK_ENABLE();
    GPIO_PinRemapConfig(GPIO_Remap_SWJ_JTAGDisable, ENABLE);
    */
    
    /* 配置CE、CSN、SCK、MOSI引脚为推挽输出 */
    ...
    HAL_GPIO_Init(NRF24L01_SCK_PORT, &GPIO_InitStruct);
    
    GPIO_InitStruct.Pin = NRF24L01_MOSI_PIN;
    HAL_GPIO_Init(NRF24L01_MOSI_PORT, &GPIO_InitStruct);
    ...
    GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_HIGH;
    HAL_GPIO_Init(NRF24L01_MISO_PORT, &GPIO_InitStruct);
