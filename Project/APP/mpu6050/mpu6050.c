...
void MPU6050_Verify_Config(void)
{
    u8 smplrt_div, config, gyro_config, accel_config;

    smplrt_div = MPU6050_Read_Byte(MPU6050_SAMPLE_RATE_REG);
    config = MPU6050_Read_Byte(MPU6050_CFG_REG);
    gyro_config = MPU6050_Read_Byte(MPU6050_GYRO_CFG_REG);
    accel_config = MPU6050_Read_Byte(MPU6050_ACCEL_CFG_REG);

    printf("MPU6050 Configuration:\n");
    printf("SMPLRT_DIV: 0x%02X (Rate: %dHz)\n", smplrt_div, 1000 / (1 + smplrt_div));
...
u8 MPU6050_Set_Rate(u16 rate)
{
    ...
    return MPU6050_Set_LPF(42); // 设置LPF为42HZ
}
...
u8 MPU6050_Write_Len(u8 addr, u8 reg, u8 len, u8 *buf)
{
    ...
    IIC_Start();
    IIC_Send_Byte((addr << 1) | 0); // 发送器件地址+写方向
...
u8 MPU6050_Read_Len(u8 addr, u8 reg, u8 len, u8 *buf)
{
    ...
    IIC_Start();
    IIC_Send_Byte((addr << 1) | 1); // 发送器件地址+读方向
    IIC_Wait_Ack();                 // 等待应答
...
{
    IIC_Start();
    IIC_Send_Byte((MPU6050_ADDR << 1) | 0); // 发送器件地址+写方向
    if (IIC_Wait_Ack()) {                    // 等待应答
        IIC_Stop();
        return 1;
    }
...
u8 MPU6050_Read_Byte(u8 reg)
{
    ...
    IIC_Start();
    IIC_Send_Byte((MPU6050_ADDR << 1) | 1); // 发送器件地址+读方向
    IIC_Wait_Ack();                          // 等待应答
