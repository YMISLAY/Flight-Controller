...
#define MPU6050_MOTION_DET_REG      0X1F    // 运动检测阈值配置寄存器
...
// IIC 地址
// 当AD0引脚(9脚)接地时,IIC地址为0X68(不含最低位).
// 如果接V3.3,则IIC地址为0X69(不含最低位).
...
void MPU6050_Calibrate_Gyro(void);          // 校准陀螺仪
void MPU6050_Verify_Config(void);           // 验证MPU6050配置
